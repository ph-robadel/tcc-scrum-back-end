		...
		if (!ObjectUtils.isEmpty(listaCapacidadeDTO)) {
			var possuiCapacidadeTimeInvalida = listaCapacidadeDTO.stream()
					.anyMatch(capacidadeTime -> ObjectUtils.isEmpty(capacidadeTime.getIdUsuario())
							|| ObjectUtils.isEmpty(capacidadeTime.getHoras())
							|| BigDecimal.ZERO.compareTo(capacidadeTime.getHoras()) >= 0);
			if (possuiCapacidadeTimeInvalida) {
				...
				if (listaUsuariosInvalidos.size() == 1) {
					erros.add("Capacidade sprint: O usuário " + listaUsuariosInvalidos.get(0)
							+ " não é membro ativo do projeto");
				} else if (listaUsuariosInvalidos.size() > 1) {
					erros.add("Capacidade sprint: Os Usuários "
		...
		if (!ObjectUtils.isEmpty(listaItensDTO)) {
			...
			if (possuiItemInvalido) {
				...
				var listaItensInvalidos = listaItensDTO.stream()
						.filter(itemDTO -> projeto.getItensBacklogValidosNovaSprint().stream()
				...
				if (listaItensInvalidos.size() == 1) {
					...
					erros.add("Item backlog inválido: Os itens "
							+ listaItensInvalidos.stream().collect(Collectors.joining(", "))
							+ " não estão listados no backlog de planejamento da sprint");
				}

				var listaUsuariosInvalidos = listaItensDTO.stream()
						.filter(itemDTO -> timeProjeto.stream()
								.noneMatch(u -> u.getId().equals(itemDTO.getIdResponsavelRealizacao())))
						.map(itemDTO -> itemDTO.getIdResponsavelRealizacao().toString()).collect(Collectors.toList());
				if (listaUsuariosInvalidos.size() == 1) {
					erros.add("Item backlog inválido: O usuário " + listaUsuariosInvalidos.get(0)
							+ " não é membro ativo do projeto");
				} else if (listaUsuariosInvalidos.size() > 1) {
					erros.add("Item backlog inválido: Os Usuários "
							+ listaUsuariosInvalidos.stream().collect(Collectors.joining(", "))
							+ " não são membros ativos do projeto");
				}
			}
